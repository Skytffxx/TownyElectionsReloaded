...
public class TownVoteGui extends VoteGui {
    ...
    public void setItems(Player player, InventoryContents contents) {
        ...
        try {
            ...
            showError(contents, "You are not part of a town.");
            return;
        }

        TownElection election = em.getTownElection(player);
        if (election == null) {
            showError(contents, "No active election in your town.");
            return;
        }
        ...
        List<TownParty> partyList = TownyElections.getInstance().getPartyManager()
                .getPartiesForTown(t.getName()).stream()
                .filter(p -> !em.isTownPartyAtTermLimit(t.getUUID(), p.getName()))
        ...
        for (int i = 0; i < items.length; i++) {
            TownParty party = partyList.get(i);
            ItemStack item = new ItemStack(Material.PAPER);
            ItemMeta meta = item.getItemMeta();
            meta.setDisplayName(ChatColor.YELLOW + party.getName());
            ...
            items[i] = ClickableItem.of(item, e -> {
                ...
                em.addPendingVote(player.getUniqueId(), partyList.get(it).getName(), true);
                player.closeInventory();
                TownConfirmVoteGui.open(player);
            });
        }
