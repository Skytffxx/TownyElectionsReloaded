    ...
    public boolean execute(Player player, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "town": {
                ...
                try {
                    party = partyManager.getPartiesForTown(resident.getTown().getName()).stream()
                            .filter(obj -> obj.getName().toLowerCase().equals(args[1]))
            ...
            case "nation": {
                ...
                try {
                    party = partyManager.getPartiesForNation(resident.getTown().getNation().getName()).stream()
                            .filter(obj -> obj.getName().toLowerCase().equals(args[1]))
                            .collect(Collectors.toList()).get(0);
                } catch (Exception e) {
        ...
        for (Player iteratedPlayer : Bukkit.getOnlinePlayers()) {
            if (party.getMembers().contains(iteratedPlayer.getUniqueId()))
                iteratedPlayer.sendMessage(TownyElections.getMessage("player-joined-the-party").replace("%player%", player.getName()));
        }
